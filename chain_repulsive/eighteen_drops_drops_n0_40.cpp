...
plint n0 = 40; // Reference resolution. dx in particle diameter
plint ly = 2; // Resolution (cylinder diameter).
plint lx = ly * 250;
...
int main(int argc, char* argv[]) {
    ...
    sprintf(outputDir, "./tmp_eighteen_w_4r_n0_40_re_0.25/");
    cout << outputDir<<endl;
   ...
   std::vector<Array<T,4> > centers_rad;
    ...
    for(int i=0; i < 18; i++){
		centers_rad.push_back({cx0+i*4*rad, cy0+0.5*rad*((int)(i==8)), rad, count_particles});
		count_particles++;
	}
    ...
    for (plint iT=0; iT < maxIter; ++iT) {
	    ...
	    for (unsigned int ind = 0; ind < all_vertices.size(); ind++) {
			...
			h = center1[1] - rad - 1;
			if (h < 2 * delta){
				pcout << iT << " " << ind << " bottom h= "<< h << " y = " << center1[1] <<endl;
				f12 = {0, - k * (2 * delta - h) };
				all_velocities[ind].updateRepulsive(f12);
			}
			// top wall crossing
			h = nY - 2 - center1[1] + rad;
			if (h < 2 * delta){
				pcout << iT << " " << ind << " top h= "<< h << " y = " << center1[1] <<endl;
				f12 = {0,  k * (2 * delta - h)};
				all_velocities[ind].updateRepulsive(f12);
			}
